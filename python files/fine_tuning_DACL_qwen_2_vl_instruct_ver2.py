...
#######TO AVOID DOWNLOADING STUFF AGAIN#######################################################################
###check-point shards of qwen are already downloaded, ###
import os
os.environ['TRANSFORMERS_CACHE'] = 'D:/mdfBIM+ - VLM 4 Bridge Damages - Jäkel_Bitte nicht löschen!_/hugging_face/hub' #If check-point shards are not downloaded, then comment it####
#######################IMPORT LIBRARIES###################################
...
peft_config = LoraConfig(
    ...
    r=16, #16 #8
    bias="none",
    target_modules=["q_proj", "k_proj", "v_proj"], #"k_proj", #"o_proj" #, "qkv", "proj"
    task_type="CAUSAL_LM",
