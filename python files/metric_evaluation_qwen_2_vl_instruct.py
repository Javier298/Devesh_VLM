...
def resize_image(image: Image.Image):
    ...
    max_size = 1120 #Ensure max 1008X1008
...
#print(val_dataset[69][1]["content"][0]["image"])
#'''
#################################################################################dataset 2######################################################################################
'''
...
adapter_path_3 = "./fine_tuned_weights/dacl"

dataset = "codebrim" #"2601506" #"codebrim"
peft_model = PeftModel.from_pretrained(model,adapter_path_1, adapter_name=dataset)

####################################################################################################
import numpy as np
from sklearn.metrics import accuracy_score, precision_recall_fscore_support
label_mapping_text_to_number = {
    "Crack": 0,
    "ACrack": 1,
    "Wetspot": 2,
    "Efflorescence": 3,
    ...
    "JTape": 17,
    "WConccor": 18,
    "Corrosion, no rust staining": 19,
    "NO Exposed Reinforcement": 20,
    "Scaling": 21,
    "General Defects": 22,
    "No defect": 23,
...
def evaluate_model(dataset):
    ...
    for sample_data in dataset:
        ...
        print("STOP!!! \n")
        #predicted_label = int(generated_answer) #label_mapping.get(generated_answer.strip(),-1)  # Default to -1 if unknown
        str_to_list = ast.literal_eval(generated_answer)
        predicted_label = str_to_list[0]
        actual_label = actual_answer #label_mapping.get(actual_answer.strip(),-1) 
